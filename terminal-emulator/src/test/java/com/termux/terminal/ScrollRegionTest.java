...
public class ScrollRegionTest extends TerminalTestCase {
  ...
  public void testDECCOLMResetsScrollMargin() {
    // DECCOLM — Select 80 or 132 Columns per Page (http://www.vt100.net/docs/vt510-rm/DECCOLM) has
    // the important
    // side effect to clear scroll margins, which is useful for e.g. the "reset" utility to clear
    // scroll margins.
    withTerminalSized(3, 4).enterString("111222333444").assertLinesAre("111", "222", "333", "444");
  ...
  public void testNELRespectsLeftMargin() {
    ...
    // The NEL (ESC E) sequence moves cursor to first position on next line, where first position
    // depends on origin mode and margin.
    withTerminalSized(3, 3)
  ...
  public void testRiRespectsLeftMargin() {
    ...
    withTerminalSized(4, 3)
        .enterString("ABCD\033[?69h\033[2;3s\033[?6h\033M")
        .assertLinesAre("A  D", " BC ", "    ");
  }

  public void testSdRespectsLeftMargin() {
    // Scroll Down (SD), ${CSI}${N}T, should respect horizontal margins:
    withTerminalSized(4, 3)
        .enterString("ABCD\033[?69h\033[2;3s\033[?6h\033[2T")
        .assertLinesAre("A  D", "    ", " BC ");
  }
  ...
  public void testClearingWhenScrollingWithMargins() {
    ...
    int newBackground = 3;
    int size = 3;
    TerminalTestCase terminal =
        withTerminalSized(size, size)
