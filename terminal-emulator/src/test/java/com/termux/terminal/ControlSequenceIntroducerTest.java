...
public class ControlSequenceIntroducerTest extends TerminalTestCase {
  ...
  public void testReportPixelSize() {
    ...
    assertEnteringStringGivesResponse(
        "\033[14t", "\033[4;" + (rows * cellHeight) + ";" + (columns * cellWidth) + "t");
   ...
   * </pre>
   *
   * <p>We currently parse the variants, but map them to normal/no underlines as appropriate
   */
  public void testUnderlineVariants() {
    for (String suffix : List.of("", ":1", ":2", ":3", ":4", ":5")) {
      for (String stop : List.of("24", "4:0")) {
        ...
        assertEquals(
            TextStyle.CHARACTER_ATTRIBUTE_BOLD | TextStyle.CHARACTER_ATTRIBUTE_UNDERLINE,
            mTerminal.mEffect);
  ...
  public void testManyParameters() {
    ...
    b.append("4:2");
    // This clearing of underline should be ignored as the parameters pass the threshold for too
    // many parameters:
    b.append("4:0m");
    withTerminalSized(3, 3).enterString(b.toString()).assertLinesAre("   ", "   ", "   ");
    assertEquals(TextStyle.CHARACTER_ATTRIBUTE_UNDERLINE, mTerminal.mEffect);
  }
}
