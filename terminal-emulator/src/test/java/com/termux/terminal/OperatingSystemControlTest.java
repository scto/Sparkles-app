...
public class OperatingSystemControlTest extends TerminalTestCase {
  ...
  public void testSetColor() throws Exception {
    ...
    assertEquals(
        Integer.toHexString(0xFF00FF00), Integer.toHexString(mTerminal.mColors.mCurrentColors[5]));
  ...
  public void testSettingDynamicColors() {
    // "${OSC}${DYNAMIC};${COLORSPEC}${BEL_OR_STRINGTERMINATOR}" => Change ${DYNAMIC} color to the
    // color specified by $COLORSPEC where:
    ...
    // ("Each successive parameter changes the next color in the list. The value of P s tells the
  ...
  public void testReportSpecialColors() {
    // "${OSC}${DYNAMIC};?${BEL}" => Terminal responds with the control sequence which would set the
    // current color.
    // Both xterm and libvte (gnome-terminal and others) use the longest color representation, which
    // means that
