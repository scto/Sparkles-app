...
public abstract class TerminalTestCase extends TestCase {
  ...
  protected TerminalTestCase withTerminalSized(int columns, int rows) {
    // The tests aren't currently using the client, so a null client will suffice, a dummy client
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      for (int j = 0; j < usedChars; j++) {
        ...
        if (Character.isHighSurrogate(c)) {
          ...
          assertFalse(
              "Low surrogate without preceding high surrogate", Character.isLowSurrogate(c));
          codePoint = c;
        }
        assertFalse(
  ...
  protected void assertLineIs(int line, String expected) {
    TerminalRow l =
        mTerminal
            .getScreen()
            .allocateFullLineIfNecessary(mTerminal.getScreen().externalToInternalRow(line));
    char[] chars = l.mText;
  ...
  public TerminalTestCase assertLineWraps(boolean... lines) {
    for (int i = 0; i < lines.length; i++)
      assertEquals(
          "line=" + i,
          lines[i],
          mTerminal.getScreen().mLines[mTerminal.getScreen().externalToInternalRow(i)].mLineWrap);
    return this;
  }

  protected TerminalTestCase assertLineStartsWith(int line, int... codePoints) {
    char[] chars =
        mTerminal.getScreen().mLines[mTerminal.getScreen().externalToInternalRow(line)].mText;
    int charIndex = 0;
