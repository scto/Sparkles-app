...
public final class TerminalRow {
   ...
   *
   * <p>There does not seem to be limit in unicode standard for max number of combination characters
   * that can be combined but such characters are primarily under 10.
   *
   * <p>"Section 3.6 Combination" of unicode standard contains combining characters info. -
   ...
   *
   * <p>UAX15-D3 Stream-Safe Text Format limits to max 30 combining characters. > The value of 30 is
   * chosen to be significantly beyond what is required for any linguistic or technical usage. >
   * While it would have been feasible to chose a smaller number, this value provides a very wide
   * margin, > yet is well within the buffer size limits of practical implementations. -
   * https://unicode.org/reports/tr15/#Stream_Safe_Text_Format -
   * https://stackoverflow.com/a/11983435/14686958
   *
   * <p>We choose the value 15 because it should be enough for terminal based applications and keep
   * the memory usage low for a terminal row, won't affect performance or cause terminal to lag or
   * hang, and will keep malicious applications from causing harm. The value can be increased if
   * ever needed for legitimate applications.
   */
  ...
  public void setChar(int columnToSet, int codePoint, long style) {
    ...
    // If MAX_COMBINING_CHARACTERS_PER_COLUMN already exist in column, then ignore adding additional
    // combining characters.
    if (newIsCombining) {
      int combiningCharsCount =
          WcWidth.zeroWidthCharsCount(
              mText, oldStartOfColumnIndex, oldStartOfColumnIndex + oldCharactersUsedForColumn);
